...
function inject (bot, { version, storageBuilder, hideErrors, rnChunkDecodeBudgetMs, rnChunkDecodeMaxQueue }) {
  ...
  // RN_PATCH: chunk decode queue v4
  // Avoid multi-second JS thread stalls by processing chunk/light packets gradually.
  // This preserves full protocol data (no packet dropping), but may delay far chunks.
  const rnNowMs = () => {
    const p = typeof globalThis !== 'undefined' ? globalThis.performance : null
    return p && typeof p.now === 'function' ? p.now() : Date.now()
  }

  const rnChunkBudgetMs = Number.isFinite(rnChunkDecodeBudgetMs) ? rnChunkDecodeBudgetMs : 6
  const rnChunkMaxQueue = Number.isFinite(rnChunkDecodeMaxQueue) ? rnChunkDecodeMaxQueue : 2048
  const rnChunkQueueEnabled = rnChunkBudgetMs > 0

  // Use distance buckets instead of sorted inserts.
  // Sorted-insert into an array becomes O(n^2) when the server streams hundreds of chunks.
  const rnBuckets = new Map()
  let rnQueued = 0
  let rnMinP = 0
  const rnLatestLightByKey = new Map()
  let rnScheduled = false

  const rnChunkCenter = () => {
    const pos = bot.entity && bot.entity.position
    if (!pos) return null
    return { x: pos.x >> 4, z: pos.z >> 4 }
  }

  const rnPriority = (x, z) => {
    const c = rnChunkCenter()
    if (!c) return 0
    return Math.abs(x - c.x) + Math.abs(z - c.z)
  }

  const rnBucketPush = (item) => {
    const p = (item.p | 0)
    item.p = p
    const bucket = rnBuckets.get(p)
    if (bucket) bucket.push(item)
    else rnBuckets.set(p, [item])
    rnQueued += 1
    if (rnQueued === 1 || p < rnMinP) rnMinP = p
  }

  const rnBucketShift = () => {
    if (rnQueued <= 0) return null
    while (rnBuckets.size) {
      const bucket = rnBuckets.get(rnMinP)
      if (bucket && bucket.length) {
        rnQueued -= 1
        const item = bucket.shift()
        if (bucket.length === 0) rnBuckets.delete(rnMinP)
        return item
      }
      rnBuckets.delete(rnMinP)
      rnMinP += 1
    }
    rnQueued = 0
    rnMinP = 0
  ...
  const rnCompactCancelled = () => {
    if (rnQueued < rnChunkMaxQueue) return
    let newQueued = 0
    let newMinP = null
    for (const [p, bucket] of rnBuckets.entries()) {
      const kept = bucket.filter((it) => it && !it.cancelled)
      if (kept.length) {
        rnBuckets.set(p, kept)
        newQueued += kept.length
        if (newMinP == null || p < newMinP) newMinP = p
      } else {
        rnBuckets.delete(p)
      }
    }
    rnQueued = newQueued
    rnMinP = newMinP == null ? 0 : newMinP
    // If still above limit, allow growth to avoid dropping protocol data.
  }

  const rnEnqueue = (item) => {
    rnCompactCancelled()
    rnBucketPush(item)
    rnSchedule()
  }

  const rnEnqueueLatestLight = (key, item) => {
    item.key = key
    const prev = rnLatestLightByKey.get(key)
    if (prev) prev.cancelled = true
    rnLatestLightByKey.set(key, item)

    rnEnqueue(item)
  }

  const rnSchedule = () => {
    if (rnScheduled) return
    rnScheduled = true
    setTimeout(rnDrain, 0)
  }

  const rnApplyUpdateLight = (packet) => {
  ...
  const rnApplyMapChunkSync = (packet) => {
    addColumn({
      x: packet.x,
      z: packet.z,
  ...
  const rnTryInitMapChunk = (item) => {
    const packet = item.packet
    if (!packet) return { done: true }

    const x = packet.x
    const z = packet.z
    const data = packet.chunkData || packet.data
    if (!Buffer.isBuffer(data)) return { done: true, fallback: true }

    // Preserve upstream semantics: empty/groundUp column means unload.
    if (!packet.bitMap && packet.groundUp) {
      delColumn(x, z)
      return { done: true }
    }

    let column = bot.world.getColumn(x, z)
    if (!column) {
      column = new Chunk({ minY: bot.game.minY, worldHeight: bot.game.height })
    }

    // Use prismarine-chunk's section/biome decoders incrementally (PC 1.18+).
    const sectionCtor = column.sections && column.sections[0] && column.sections[0].constructor
    const biomeCtor = column.biomes && column.biomes[0] && column.biomes[0].constructor
    const noSizePrefix = !!(column.sections && column.sections[0] && column.sections[0].noSizePrefix)
    if (!sectionCtor || typeof sectionCtor.read !== 'function' || !biomeCtor || typeof biomeCtor.read !== 'function') {
      return { done: true, fallback: true }
    }

    const SmartBuffer = require('smart-buffer').SmartBuffer
    item.column = column
    item.reader = SmartBuffer.fromBuffer(data)
    item.sectionCtor = sectionCtor
    item.biomeCtor = biomeCtor
    item.noSizePrefix = noSizePrefix
    item.numSections = typeof column.numSections === 'number' ? column.numSections : (bot.game.height >> 4)
    item.sectionIndex = 0
    item.maxBitsPerBlock = column.maxBitsPerBlock
    item.maxBitsPerBiome = column.maxBitsPerBiome

    return { done: false }
  }

  const rnFinalizeMapChunk = (packet, column) => {
    if (packet.biomes !== undefined && typeof column.loadBiomes === 'function') {
      column.loadBiomes(packet.biomes)
    }
    if (packet.skyLight !== undefined && typeof column.loadParsedLight === 'function') {
      column.loadParsedLight(packet.skyLight, packet.blockLight, packet.skyLightMask, packet.blockLightMask, packet.emptySkyLightMask, packet.emptyBlockLightMask)
    }
    bot.world.setColumn(packet.x, packet.z, column)

    if (typeof packet.blockEntities !== 'undefined') {
      const col = bot.world.getColumn(packet.x, packet.z)
      if (!col) {
        if (!hideErrors) console.warn('Ignoring block entities as chunk failed to load at', packet.x, packet.z)
        return
      }
      for (const blockEntity of packet.blockEntities) {
        if (blockEntity.x !== undefined) { // 1.17+
          col.setBlockEntity(blockEntity, blockEntity.nbtData)
        } else {
          const pos = new Vec3(blockEntity.value.x.value & 0xf, blockEntity.value.y.value, blockEntity.value.z.value & 0xf)
          col.setBlockEntity(pos, blockEntity)
        }
      }
    }
  }

  const rnStepMapChunk = (item) => {
    const packet = item.packet
    if (!packet) return true

    if (!item.reader) {
      const init = rnTryInitMapChunk(item)
      if (init.fallback) { rnApplyMapChunkSync(packet); return true }
      if (init.done) return true
    }

    // Decode a small amount of work per drain turn.
    const perTurn = 1
    let count = 0
    while (item.sectionIndex < item.numSections && count < perTurn) {
      const i = item.sectionIndex
      const t0 = rnNowMs()
      item.column.sections[i] = item.sectionCtor.read(item.reader, item.maxBitsPerBlock, item.noSizePrefix)
      const t1 = rnNowMs()
      item.column.biomes[i] = item.biomeCtor.read(item.reader, item.maxBitsPerBiome, item.noSizePrefix)
      const t2 = rnNowMs()
      if (typeof __DEV__ !== 'undefined' && __DEV__ && (t2 - t0) > 50) {
        console.log('[mineflayer] slow chunk section decode', {
          ms: t2 - t0,
          sectionMs: t1 - t0,
          biomeMs: t2 - t1,
          x: packet.x,
    ...
    if (item.sectionIndex < item.numSections) return false

    try {
      rnFinalizeMapChunk(packet, item.column)
    } catch (e) {
      bot.emit('error', e)
    }
    return true
  ...
  const rnDrain = () => {
    ...
    while (rnQueued && (rnNowMs() - start) < rnChunkBudgetMs) {
      const item = rnBucketShift()
      if (!item || item.cancelled) continue
      const key = item.key
      try {
        if (item.t === 'update_light') {
          rnApplyUpdateLight(item.packet)
        } else if (item.t === 'map_chunk') {
          const done = rnStepMapChunk(item)
          if (!done) {
            item.p = rnPriority(item.packet.x, item.packet.z)
            rnBucketPush(item)
          }
        } else if (item.t === 'map_chunk_bulk') {
          const packet = item.packet
          if (!packet || !packet.meta || !packet.data) continue
          if (item.i == null) item.i = 0
          if (item.offset == null) item.offset = 0
          const meta = packet.meta[item.i]
          if (!meta) {
            // finished
            continue
          }
          ...
          addColumn({
            ...
            groundUp: true,
            data: packet.data.slice(item.offset, item.offset + size)
          })
          item.offset += size
          item.i += 1
          // Requeue until complete.
          item.p = rnPriority(meta.x, meta.z)
          rnBucketPush(item)
        }
      } catch (e) {
        bot.emit('error', e)
      } finally {
        if (key && item.t === 'update_light') {
          if (rnLatestLightByKey.get(key) === item) rnLatestLightByKey.delete(key)
        }
      }
    }

    if (rnQueued) rnSchedule()
  }

  const rnQueueUpdateLight = (packet) => rnEnqueueLatestLight(
    `light:${packet.chunkX},${packet.chunkZ}`,
    { t: 'update_light', packet, p: rnPriority(packet.chunkX, packet.chunkZ), cancelled: false, key: null }
  )
  const rnQueueMapChunk = (packet) => rnEnqueue({ t: 'map_chunk', packet, p: rnPriority(packet.x, packet.z), cancelled: false, key: null })
  const rnQueueMapChunkBulk = (packet) => {
    const first = packet && packet.meta && packet.meta[0]
    const p = first ? rnPriority(first.x, first.z) : 0
    rnEnqueue({ t: 'map_chunk_bulk', packet, p, cancelled: false, key: null, i: 0, offset: 0 })
  }
  ...
  bot._client.on('update_light', (packet) => {
    if (rnChunkQueueEnabled) { rnQueueUpdateLight(packet); return }
    let column = bot.world.getColumn(packet.chunkX, packet.chunkZ)
    if (!column) {
      column = new Chunk({ minY: bot.game.minY, worldHeight: bot.game.height })
      bot.world.setColumn(packet.chunkX, packet.chunkZ, column)
  ...
  bot._client.on('map_chunk', (packet) => {
    if (rnChunkQueueEnabled) { rnQueueMapChunk(packet); return }
    addColumn({
      x: packet.x,
      z: packet.z,
      bitMap: packet.bitMap,
  ...
  bot._client.on('map_chunk_bulk', (packet) => {
    if (rnChunkQueueEnabled) { rnQueueMapChunkBulk(packet); return }
    let offset = 0
    let meta
    let i
    let size
